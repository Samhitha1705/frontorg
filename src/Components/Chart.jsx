...
export default function Chart() {
  ...
  const [employeeId, setEmployeeId] = useState("1002"); // Initial employee ID
    ...
    const fetchData = async () => {
      try {
        ...
        const [originResponse] = await Promise.all([
          axios.get(`https://orga123.azurewebsites.net/api/employees/origin/${employeeId}`)
        ]);
      ...
      try {
        ...
        const [ employeesResponse] = await Promise.all([
          axios.get("https://orga123.azurewebsites.net/api/employees")
        ]);
      ...
      try {
        ...
        const [ reportingResponse] = await Promise.all([
          axios.get(`https://orga123.azurewebsites.net/api/employees/reporting-to/${employeeId}`)
        ]);
